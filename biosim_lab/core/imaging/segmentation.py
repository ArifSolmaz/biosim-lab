...
def segment_classical(
    ...
    *,
    ...
    1. Background subtraction removes the illumination gradient: a wide
       Gaussian of the image itself, or a given *background* frame.
    ...
    threshold:
        Fixed threshold; ``None`` uses Otsu. For a video, fix it once: in a
        frame with no whole cell in view Otsu splits pure noise.
    background:
        A static background frame (e.g. the temporal median of a video) to
        subtract instead of the Gaussian estimate --- exact for a fixed
        camera, and much faster than a wide blur per frame.
    """
    ...
    static_background = background is not None
    if background is None:
        background = gaussian(img, sigma=background_sigma, preserve_range=True)
    elif np.shape(background) != img.shape:
        raise ValueError(f"background shape {np.shape(background)} != image shape {img.shape}")
    flat = img - background
    ...
    parameters = {
        ...
        "background_sigma": None if static_background else background_sigma,
        "threshold": level,
        "clear_border": clear_border,
    }
    labels = np.zeros(img.shape, dtype=np.int32)
    if not mask.any():  # nothing but debris: skip the watershed
        return SegmentationResult(labels=labels, properties=_measure(labels, img),
                                  backend="classical", parameters=parameters)

    # Work inside the mask's bounding box plus a one-pixel ring. Everything
    # outside is background, so hole filling, the distance transform and the
    # watershed come out identical --- only cheaper on a sparse frame (a video
    # of a few cells in a wide channel).
    rows = np.flatnonzero(mask.any(axis=1))
    cols = np.flatnonzero(mask.any(axis=0))
    box = (slice(max(rows[0] - 1, 0), rows[-1] + 2), slice(max(cols[0] - 1, 0), cols[-1] + 2))
    sub = _fill_holes(mask[box])
    ...
    for i, (r, c) in enumerate(seeds_coords, start=1):
        markers[r, c] = i
    labels[box] = watershed(-distance, markers, mask=sub)
    ...
    return SegmentationResult(
        labels=labels, properties=props, backend="classical", parameters=parameters,
    )


def _fill_holes(mask: np.ndarray) -> np.ndarray:
    """``scipy.ndimage.binary_fill_holes`` in one labelling pass.

    A hole is a background region (4-connected, as scipy's default) that does
    not reach the image edge. scipy finds them by dilating from the edge until
    nothing changes, which costs one full-image pass per pixel of the widest
    background path.
    """
    from scipy import ndimage as ndi

    background, _ = ndi.label(~mask)
    edge = np.unique(np.concatenate([background[0], background[-1],
                                     background[:, 0], background[:, -1]]))
    return mask | ~np.isin(background, edge[edge > 0])
